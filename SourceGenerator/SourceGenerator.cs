    ...
    public class HelloSourceGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                list.Add(JsonConvert.SerializeObject(new { Name = name, Text = File.ReadAllText(path) }));  
            }
...
{{
    ...
    {{
        List<string> list = JsonSerializer.Deserialize<List<string>>(""{JsonConvert.SerializeObject(list)}"");
...
";
            context.AddSource($"AutoDownload1.g.cs", source);
