    ...
    public class HelloSourceGenerator : ISourceGenerator
    {
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                // (предложенная в условии сериализация без нугетов не катит, тк генератор поддерживает только 2ая версия нета, а та сериализация появилась только в 3 версии)
                if (i != 0)
                {
                    listNodes += ",";
                }
